<template>
  <div class="table-list">
    ...
    <el-table :data="tableData" @row-dblclick="edit" @selection-change="selectChange" :default-sort="orderInfo" @sort-change="sortChange" border highlight-current-row style="width: 100%">
      ...
      <el-table-column prop="role_key" label="角色标识" width="150"></el-table-column>
      <el-table-column prop="role_name" label="角色名称" width="150"></el-table-column>
      <el-table-column prop="describe" label="角色描述" min-width="120"></el-table-column>
...
export default {
  ...
  created: function () {
    bus.$on(role.search, (searchData) => { // 监听外部查询数据事件
      this.search(searchData)
    })
    bus.$on(role.refreshListForEdit, () => { // 监听数据更改后的列表刷新（刷新当前页）
      this.getData()
    })
    bus.$on(role.refreshListForAdd, () => { // 监听数据添加后的列表刷新
      this.resetPageInfo()
  ...
  data: function () {
    let pageSize = localStorage.getItem(localStorageKeys.rolePageSize)
    return {
...
//    },
    ...
    getData: function () { // 获取服务器数据
      this.$http.post('/manage/role/getData', Object.assign({}, this.searchData, this.pageInfo, this.orderInfo)).then((response) => {
    ...
    deleteRec: function (selectIds) { // 删除数据
      ...
      this.$confirm('确认要删除数据（不可恢复）?', '提示', {
        ...
        this.$http.post('/manage/role/delete', {ids: selectIds}).then((response) => {
          this.getData()
        })
      }).catch(() => { })
    },
    ...
    handleSizeChange: function (val) { // 每页条数改变时重新加载记录
      ...
      localStorage.setItem(localStorageKeys.rolePageSize, val)
      this.getData()
    },
