<template>
  <div class="table-list">
    ...
    <el-table :data="tableData" @row-dblclick="edit" @selection-change="selectChange" :default-sort="orderInfo" @sort-change="sortChange" border highlight-current-row style="width: 100%">
      ...
      <el-table-column prop="name" label="分类标签名称"  min-width="100">
...
export default {
  ...
  created: function () {
    bus.$on(typeGroup.search, (searchData) => { // 监听外部查询数据事件
      this.search(searchData)
    })
    bus.$on(typeGroup.refreshListForAdd, () => { // 监听数据添加后的列表刷新
      this.resetPageInfo()
  ...
  data: function () {
    let pageSize = localStorage.getItem(localStorageKeys.typeGroupPageSize)
    return {
  ...
  methods: {
    ...
    getData: function () { // 获取服务器数据
      this.$http.post('/manage/typeGroup/getData', Object.assign({}, this.searchData, this.pageInfo, this.orderInfo)).then((response) => {
    ...
    setStatus: function (status, selectIds) { // 请求服务器设置状态
      ...
      if (selectIds === null || selectIds.length === 0) {
        this.$message({type: 'warning', message: '选择一个分类标签进行更新状态'})
        return false
      }
      this.$http.post('/manage/typeGroup/changeStatus', {ids: selectIds, status: status}).then((response) => {
        this.getData()
      })
    },
    ...
    saveEdit: function (row) {
      if (row.name === '') {
        this.$message({type: 'warning', message: '请输入分类标签名字'})
        return false
      }
      if (row.sort === '') {
      ...
      let editData = Object.assign({id: row.id, status: row.status, name: row.name, sort: row.sort})
      this.$http.post('/manage/typeGroup/save', editData).then((response) => {
        this.$message({type: 'success', message: '修改数据成功'})
        this.getData()
      })
    },
    add: function () { // 添加记录
//      bus.$emit(typeGroup.showAdd)
      this.editCompName = 'tableForm'
      this.showEdit = true
    },
    handleSizeChange: function (val) { // 每页条数改变时重新加载记录
      this.pageInfo.pageSize = val
      localStorage.setItem(localStorageKeys.typeGroupPageSize, val)
      this.getData()
    },
