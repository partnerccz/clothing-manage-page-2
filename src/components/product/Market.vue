  ...
  methods: {
    ...
    show: function () { // 首次显示dialog时，show方法不会执行，所以需要在mounted中调用，后续直接通过dialog的open方法调用
      ...
      if (this.editRowId !== null) {
        this.$http.post('/productMarket/info', {productId: this.editRowId}, {showLoading: true}).then((response) => {
          let imgs = []
...
//        url: "blob:http://localhost:8085/64453705-8cbc-4b97-9793-76e944cd5921"
        this.productMarket.imgs.push({url: img.response || img.url}) // 这里的处理是兼容element-ui的数据结构，element-ui上传后的数据结构如上，url为本地图片地址，response才是服务器图片地址
      }
    },
    onSubmit: function () {
